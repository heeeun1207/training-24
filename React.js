// 1. 본 예제에서 diff의 논리방식을 추론, 이해하기
// 2.  더 나아가 타입스크립트 방식으로 이식해보는 연습을 진행하기

/**
 * @param {array} targetArray
 * @param {array} compareArray
 * @param {array} originalArray
 * @returns {object} 중복된 요소와 , 바뀔요소를 담은 객체
 * 
...
function diffFinder (targetArray,compareArray,originalArray){
  //복사할 요소와, 교체할 요소를 별도의 메모리(변수)에 담아놓기 위해 null 초기화
  //이렇게 지역변수로 '이름'을 작성해 무엇을 염두하고 있는지를 유추할 수 있습니다.
  let duplicatedElemnet = null;
  ...
  // 하지만 아래의 실행에 의해 string 으로 변활될 예정이기 때문에 에러가 발생할 것이 확정입니다.
  // 타입 명시 , 타입 표기도 약간의 변용 여지를 남겨주기위해 여러 타임을 명시하는 것을
  // 합집합 타입, 유니온 타입이라고 합니다. (union type)

  targetArray.forEach((element,index)=> {
    if (compareArray.includes(element) === true) {
      duplicatedElemnet = element; //중복되어 있는 요소를 미리 만들어둔 변수에 할당
      replaceElement = originalArray[index]; // 중복되었다고 판단되는 시점의 index를 통해 교체하기 위해 별도의 변수에 할당 
      targetArray[index] = replaceElement //요소 교체
    }
  });
//리턴 부분에서 가독성을 위해 객체의 키값을 지역변수이름과 동일하게 지정하였지만,
//완전히 다른 키이름으로 지정해도 무방하다.
  return {
    duplicatedElemnet : duplicatedElemnet,
